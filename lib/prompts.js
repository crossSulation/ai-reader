 ...
 *   6. 可选带上整页正文（读文档时「上文提到的那个概念」才有处可查）。上限与截断
 *      规则都在 lib/page-text.js，本文件只负责把它拼进提示词并如实标注截断。
 */

import { pageMax } from './page-text.js';
 ...
 * 「整页正文」在提示词里的标题。
 *
 * 带上「仅作背景」这句是有原因的：不加限定的话，模型看到一整篇文档会倾向于
 * 综述全文，而用户真正问的只是划中的那一句。
 */
const PAGE_TEXT_TITLE = '【整页正文（仅作背景参考，要回答的是下面选中的内容）】';

/**
 * 正文被截断时的标注。
 * 必须写出来 —— 没写的话模型会把「我看到的正文」当成「全部正文」，
 * 进而在用户问「这篇文档还讲了什么」时给出一个自以为完整的答案。
 */
function pageTextNote(kept, total) {
  return `（以上正文未能全部放入，仅取前 ${kept} 字，过滤后正文共约 ${total} 字）`;
}

/**
 * 组装「整页正文」那一段，并回报它实际占了多少字。
 * 上限走 lib/page-text.js 的 pageMax：既受硬上限约束，也绝不会吃掉超过四成预算。
 */
function pageTextPart(pageText, charBudget) {
  const raw = String((pageText && pageText.text) != null ? pageText.text : pageText || '').trim();
  if (!raw) return { block: '', info: null };

  const cap = pageMax(charBudget);
  const rawTotal = Math.max(Number(pageText && pageText.totalChars) || 0, raw.length);
  const over = raw.length > cap;
  const body = over ? raw.slice(0, cap) : raw;
  // 采集侧已经按硬上限截过一次的情况也要如实标注（否则「截断」会被静默吞掉）
  const clipped = over || rawTotal > raw.length;

  const block = clipped
    ? `${PAGE_TEXT_TITLE}\n${body}\n${pageTextNote(body.length, rawTotal)}`
    : `${PAGE_TEXT_TITLE}\n${body}`;

  return { block, info: { chars: body.length, totalChars: rawTotal, clipped } };
}
 ...
 * @param {object|string} [p.pageText] 整页正文。对象形态见下方说明，也接受裸字符串
 * @returns {{ messages: Array, contextInfo: object }}
 *   contextInfo 描述历史被如何压缩，供面板显示「本轮带了多少上下文」：
 *   { budget, totalTurns, fullTurns, summarizedTurns, omittedTurns, pageText }
 *   pageText 为 null 表示本轮没带整页正文；否则 { chars, totalChars, clipped }，
 *   面板据此提示「带上了多少」以及「正文被截断了」。
 */
export function buildRequest({
  ...
  // 整页正文紧跟所在段落：两者都是「背景」，由近及远；选中内容紧随其后，
  // 免得几千字的背景把真正要处理的那句话挤到很远的地方去
  const pagePart = pageTextPart(pageText, charBudget);
  if (pagePart.block) lines.push(pagePart.block);
  lines.push(`【用户选中的内容】\n<<<SELECTION\n${sel}\nSELECTION>>>`);
