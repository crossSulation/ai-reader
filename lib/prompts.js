...
function planHistory(turns, budget) {
  ...
  if (!older.length) return { recent: recentMessages, summary: '' };
  ...
  return { recent: recentMessages, summary: parts.join('\n') };
}
 ...
 *                              份量控制在本函数内按 TOTAL_CHAR_BUDGET 完成
...
export function buildMessages({
  ...
  const baseCost = SYSTEM_PROMPT.length + lines.join('\n\n').length;
  const { recent, summary } = planHistory(turns, TOTAL_CHAR_BUDGET - baseCost);
