...
(() => {
  ...
  const MESSAGES = {
    zh: {
      ...
      ctxPageNote: '本轮已带上整页正文（{n} 字）',
      ctxPageClipped: '整页正文过长：本轮只带了前 {n} 字（过滤后正文共约 {total} 字）',
      ...
      optPageContext: '页面上下文',
      optPageContextHint:
        '默认只把你划中的内容与所在段落发出去。<b>开启后，当前页面的正文也会一并发给你配置的模型服务</b>（最多 8000 字，且不超过预算的四成），长文档里问「上面提到的那个概念」会准得多。「自动」只在所在段落太短时才补整页。改动立即生效。',
      optPageCtxOff: '仅所在段落（默认，最省）',
      optPageCtxAuto: '所在段落不足时自动补整页',
      optPageCtxAlways: '总是带上整页正文',
    ...
    en: {
      ...
      ctxPageNote: 'This turn included the page text ({n} characters)',
      ctxPageClipped:
        'Page text was too long: only the first {n} characters went out (about {total} in total)',
      ...
      optPageContext: 'Page context',
      optPageContextHint:
        'By default only your selection and its paragraph are sent. <b>Turning this on also sends the current page text to the model provider you configured</b> (up to 8,000 characters, never more than 40% of the budget) — far better at "that concept mentioned above" in long documents. "Auto" only falls back to the full text when the paragraph is too thin. Takes effect immediately.',
      optPageCtxOff: 'Paragraph only (default, lightest)',
      optPageCtxAuto: 'Full page text when the paragraph is thin',
      optPageCtxAlways: 'Always include the full page text',
